...
# PRISM2 - 宏微尺度几何分歧感知的概念漂移适应框架（优化后的默认参数）
# 注意：实际使用时会从settings.py获取针对特定模型和数据集的优化配置
parser.add_argument('--prism2_window_size', type=int, default=None, help='PRISM2滑动窗口大小 (None=使用优化配置)')
parser.add_argument('--prism2_k_neighbors', type=int, default=None, help='PRISM2 k近邻数量')
parser.add_argument('--prism2_lid_k', type=int, default=None, help='PRISM2 LID估计的k值')
parser.add_argument('--prism2_gamma', type=float, default=None, help='PRISM2前驱模式增强系数')
parser.add_argument('--prism2_tau_R', type=float, default=None, help='PRISM2微尺度异常阈值')
parser.add_argument('--prism2_tau_H', type=float, default=None, help='PRISM2宏尺度稳定阈值')
parser.add_argument('--prism2_ema_alpha', type=float, default=None, help='PRISM2 EMA系数')
parser.add_argument('--prism2_theta_epsilon', type=float, default=None, help='PRISM2分歧阈值(触发PRECURSOR)')
parser.add_argument('--prism2_theta_H', type=float, default=None, help='PRISM2宏尺度变化阈值(触发DRIFT)')
parser.add_argument('--prism2_precursor_lr', type=float, default=None, help='PRISM2 PRECURSOR阶段学习率')
parser.add_argument('--prism2_precursor_neighbors', type=int, default=None, help='PRISM2 PRECURSOR检索样本数')
parser.add_argument('--prism2_drift_lr', type=float, default=None, help='PRISM2 DRIFT阶段学习率')
parser.add_argument('--prism2_drift_neighbors', type=int, default=None, help='PRISM2 DRIFT检索样本数')
parser.add_argument('--prism2_drift_steps', type=int, default=None, help='PRISM2 DRIFT更新步数')
parser.add_argument('--prism2_weight_lambda', type=float, default=None, help='PRISM2流形感知权重系数')
parser.add_argument('--prism2_warmup_steps', type=int, default=None, help='PRISM2预热步数')
parser.add_argument('--prism2_stable_update_interval', type=int, default=None, help='PRISM2 STABLE状态每N步更新一次')
parser.add_argument('--prism2_stable_lr_scale', type=float, default=None, help='PRISM2 STABLE状态学习率缩放因子')
...
# 从data_path中提取数据类型，支持带路径前缀的情况
_data_filename = args.data_path.split('/')[-1]  # 获取文件名
if _data_filename.startswith('ETT'):
    args.data = _data_filename[:5]  # 例如 'ETTh1', 'ETTm2'
else:
    args.data = 'custom'
...
if args.online_method:
    ...
    if 'FSNet' in args.model:
        args.pretrain = False
    elif args.online_method.lower() == 'prism2':
        # PRISM2: 支持从头训练或加载预训练模型
        # 如果用户没有显式指定pretrain，则不强制预训练
...
if platform.system() != 'Windows':
    ...
    path = './'
    # 使用本地checkpoints目录，如果需要使用D盘请取消下面的注释
    # if args.checkpoints:
...
if args.online_method:
    ...
    if 'prism2' in flag.lower():
        # PRISM2特定的flag标识
        flag += f'_ws{args.prism2_window_size}'
        flag += f'_te{args.prism2_theta_epsilon}'
        flag += f'_th{args.prism2_theta_H}'
        if not args.freeze:
            flag += "_fulltune"
        if not args.pretrain:
            flag += "_new"
...
if __name__ == '__main__':
    ...
    for ii in range(args.itr):
        ...
        if args.online_method not in ['Online', 'SOLID', 'ER', 'DERpp']:
            ...
            # Skip training if:
            # 1. only_test/do_valid is True AND the method-specific checkpoint exists, OR
            # 2. only_test is True AND pretrain is True (weights already loaded in _build_model)
            if (args.only_test or args.do_valid) and (os.path.exists(path) or args.pretrain):
                if os.path.exists(path):
                    print('Loading', path)
                    exp.load_checkpoint(path)
                    print('Learning rate of model_optim is', exp.model_optim.param_groups[0]['lr'])
                else:
                    print('Using pretrained weights (loaded during model build)')
            else:
