...
class Exp_Basic(object):
    ...
    def load_state_dict(self, state_dict: typing.OrderedDict[str, torch.Tensor], model=None, strict=False) -> nn.Module:
        ...
        if model is None:
            model = self.model
        print(f"Exp_Basic.load_state_dict: model type = {type(model).__name__}")
        if 'model_optim' not in state_dict:
            processed_state = remove_state_key_prefix(state_dict, model)
            print(f"Exp_Basic.load_state_dict: state_dict keys (first 3): {list(processed_state.keys())[:3]}")
            model.load_state_dict(processed_state, strict=strict)
        else:
            for k, v in state_dict.items():
                if k == 'model':
                    processed_state = remove_state_key_prefix(v, model)
                    print(f"Exp_Basic.load_state_dict: model state_dict keys (first 3): {list(processed_state.keys())[:3]}")
                    model.load_state_dict(processed_state, strict=strict)
